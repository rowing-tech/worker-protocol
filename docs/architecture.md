...
> health, metrics, the Actions it accepts, and the Tasks and Alerts it has raised.
>
...
flowchart LR
  subgraph hub["The Hub — registry & operator console"]
    ...
    console["operator UI:<br/>health, metrics, alerts, actions"]
  end
  source["Upstream source"]
  caller["Any caller"]
  worker["Worker that talks to sources"]
  app["Worker that talks to people"]
  broker["Broker"]
  person["A person"]
  ...
  registry -. "brokers a Contract" .-> worker
```
...
Schema: its **health**, the minimal answer to a poll, whose absence is the signal; its **metrics**,
values exposed for reading — a thousand pending tasks, the oldest three days old; its **Actions**,
each with a schema and an address to post to; its **Tasks**, exposed as current state so a late
consumer sees what is open and not only what happened; and its **Alerts**. The hub is one client of
this API; other workers are the main ones.

**Health, concretely.** The shape is the one the cloud platforms and the IETF health-check draft
converge on: one top-level status with three values — `healthy`, `degraded`, `unhealthy` — and a
map of named checks, each with its own status and a short human-readable detail: the upstream
...
settles contention by first commit, and the owner has no policy about it. A retry cap and a crash
ceiling live on the Claim, never on the Task: a Task that outlives several failed Claims is a
signal, not a stuck queue.
...
exposes health and metrics to the hub: how many vehicles reported, how many are silent. It raises
...
| **Worker API** | What a Worker answers when polled, over HTTP and JSON Schema: its health, its metrics, the Actions it accepts, and the Tasks and Alerts it has raised. |
