...
applies everywhere else: something is produced and a party reads it. A report listing forty-one
checks, all green, over a protocol of ninety-eight rules tells an operator that a Worker was
checked against the protocol. It was checked against forty-two percent of it, and nothing on the
page says which part — so the reader concludes more than was established, which is the same fault as
a generated artifact nobody compares.
...
`other subject` is the verdict for the twenty-one rules that bind somebody who is not a Worker.
