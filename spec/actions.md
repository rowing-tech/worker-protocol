...
Capability names of DESC-14 are. A console that can show *this Worker's settings* for a Worker
nobody told it about needs one name to look for, and a name a console matches against a name that
came from elsewhere is exactly what a reserved word is for. Unlike DESC-14 there is no syntax to
draw the line with, because Action names are never compared between Workers and need none: the
