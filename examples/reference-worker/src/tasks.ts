...
import type { taskPage, taskTypeDeclaration } from "@worker-protocol/schemas";
...
export const ANSWERS: string[] = [`${NAMESPACE}.verify-vehicle`];
...
export function createTasks() {
  ...
  return {
    ...
    read(query: URLSearchParams, covers?: string[]): Refusal | z.infer<typeof taskPage> {
    ...
    write(query: URLSearchParams): Refusal | Answer {
      ...
      if (claimId === null) {
        return reject("invalid_parameter", "A write names a Task or a Claim.");
      }
